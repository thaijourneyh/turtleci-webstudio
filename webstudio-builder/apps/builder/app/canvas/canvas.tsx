...
const projectMetaCodeMarkerAttribute = "data-ws-project-meta-code";

const ProjectMetaCode = () => {
  const pages = useStore($pages);
  const projectCode = pages?.meta?.code ?? "";

  useLayoutEffect(() => {
    const removeExisting = () => {
      for (const node of document.head.querySelectorAll(
        `[${projectMetaCodeMarkerAttribute}]`
      )) {
        node.remove();
      }
    };
    ...
    template.innerHTML = projectCode;
    ...
    for (const child of Array.from(template.content.childNodes)) {
      ...
      if (child.tagName === "SCRIPT") {
        ...
        nodeToAppend = child.cloneNode(true) as Element;
      }

      nodeToAppend.setAttribute(projectMetaCodeMarkerAttribute, "");
      document.head.appendChild(nodeToAppend);
      appendedNodes.push(nodeToAppend);
    }
